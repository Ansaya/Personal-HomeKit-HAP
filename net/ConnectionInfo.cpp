...
static const unsigned char generator[] = { 0x05 };
...
void ConnectionInfo::handlePairSetup() {
    identity[36] = 0;
    MessageDataRecord stateRecord;
    stateRecord.activate = true;
    stateRecord.data = new char[1];
    stateRecord.length = 1;
    stateRecord.index = 6;
    PairSetupState state = State_M1_SRPStartRequest;
    SRP *srp;
        ...
        *(stateRecord.data) = (char)state+1;
        switch (state) {
            ...
            case State_M3_SRPVerifyRequest: {
                ...
                if (!SRP_OK(result)) {
                    MessageDataRecord responseRecord;
            ...
            case State_M5_ExchangeRequest: {
                ...
                Poly1305_GenKey((const unsigned char*)temp2, (unsigned char *)encryptedData, packageLen - 16, false, verify);
                ...
                if (bcmp(verify, mac, 16)) {
                     ...
                     * HAK Pair Setup M6
                     */
                    MessageData *subTLV8 = new MessageData(decryptedData, packageLen-16);
    ...
    do {
        ...
        switch (state) {
            case State_Pair_Verify_M1: {
                ...
                MessageDataRecord pubKeyRecord;
                ...
                char verify[16];
                memset(verify, 0, 16);
                Poly1305_GenKey((const unsigned char *)polyKey, (uint8_t *)encryptMsg, msgLen, false, verify);
                memcpy((unsigned char *)&encryptMsg[msgLen], verify, 16);
                
                MessageDataRecord encryptRecord;
            ...
            case State_Pair_Verify_M3: {
                ...
                char verify[16];    bzero(verify, 16);
                Poly1305_GenKey((const unsigned char *)temp2, (uint8_t *)encryptedData, packageLen - 16, false, verify);
                ...
                if (!bcmp(verify, &encryptedData[packageLen-16], 16)) {
                    ...
                    MessageData data(decryptData, packageLen-16);
                    
                    KeyRecord rec = KeyController::getInstance().getControllerKey(data.dataPtrForIndex(1));
...
void ConnectionInfo::handleAccessoryRequest() {
    ...
    do {
        ...
        if (len > 0) {
            ...
            //Ploy1305 key
            char verify[16];    bzero(verify, 16);
            Poly1305_GenKey((const unsigned char *)temp2, (uint8_t *)buffer, msgLen, true, verify);
            ...
            HAPService::getInstance().handleAccessory(decryptData, msgLen, &resultData, &resultLen, this);
            ...
            Poly1305_GenKey((const unsigned char *)temp2, (uint8_t *)reply, resultLen, true, verify);
